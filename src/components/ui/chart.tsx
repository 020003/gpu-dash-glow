...
>(({ id, className, children, config, responsiveDebounce = 0, ...props }, ref) => {
...
const ChartStyleBase = ({ id, config }: { id: string; config: ChartConfig }) => {
...
const ChartStyle = React.memo(
  ChartStyleBase,
  (prev, next) => prev.id === next.id && prev.config === next.config
)
